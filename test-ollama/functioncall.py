...
def generate_response(
    prompt: str, with_functions: bool = True, request_json: bool = False
):
    """
    Generate a response using the Ollama API with command-r7b model
    ...
    if with_functions:
        ...
        # Add tool_choice to force function calling if request_json is True
        if request_json:
...
def process_user_input(prompt: str, history: list):
    """Process user input with function calling"""
    print("Response: ", end="", flush=True)

    # Call the model to check if we need to call a function
    response = generate_response(prompt, with_functions=True, request_json=True)
    ...
    # Don't print the raw response immediately
    # Instead, check if it contains a function call first
...
def main():
    print("Interactive Ollama Chat (with OpenAI-style function calls)")
