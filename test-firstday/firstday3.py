...
def fetch_first_day_data():
    ...
    for single_date in daterange(datetime(start_year, start_month, start_day, tzinfo=timezone.utc), end_date):
        ...
        if first_block and balance:
            write_to_console_and_file(f"Date: {year}-{month:02d}-{day:02d}, First Block: {first_block} ( {hex_first_block}), Balance: {balance_eth} ({balance}) Address: {crc_address} / {eth_address}")
        else:
