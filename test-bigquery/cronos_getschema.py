...
def main():
    # Parse the full dataset reference
    full_dataset_ref = f"{project_id}.public_preview___blockchain_analytics_cronos_mainnet"
    project_id_override, dataset_id = full_dataset_ref.split('.', 1)
    
    # Use the project from the dataset reference instead of env var for this dataset
    client_override = bigquery.Client(project=project_id_override)

    try:
        # Get all tables in the dataset
        print(f"# BigQuery Dataset Schema: {full_dataset_ref}\n")
        dataset_ref = client_override.dataset(dataset_id)
        tables = list(client_override.list_tables(dataset_ref))
        table_ids = [table.table_id for table in tables]

        if not table_ids:
            print(f"❌ No tables found in dataset {full_dataset_ref}")
            return
        ...
        for table_id in table_ids:
            try:
                print_table_schema_markdown_override(client_override, dataset_id, table_id)
            except Exception as e:
                print(f"\n## ❌ Error: {table_id}\n")
        ...
        generate_markdown_file_override(client_override, dataset_id, table_ids, full_dataset_ref)
        print("✅ Schema saved to `cronos_schemas.md`")

        # Also save schemas to JSON file for programmatic access
        schemas = {}
        for table_id in table_ids:
            try:
                table_ref = client_override.dataset(dataset_id).table(table_id)
                table = client_override.get_table(table_ref)
                schemas[table_id] = {
        ...
        # Save to JSON file
        with open("cronos_schemas.json", "w") as f:
            json.dump(schemas, f, indent=2, default=str)
        print("✅ Schema also saved to `cronos_schemas.json`")

    except Exception as e:
        print(f"❌ **Error:** {e}")


def print_table_schema_markdown_override(client_override, dataset_id, table_id):
    """Print the schema of a specific table in markdown format with custom client"""
    table_ref = client_override.dataset(dataset_id).table(table_id)
    table = client_override.get_table(table_ref)
...
def generate_markdown_file_override(client_override, dataset_id, tables, full_dataset_ref):
    """Generate a complete markdown file with all schemas using custom client"""
    with open("cronos_schemas.md", "w") as f:
        f.write(f"# BigQuery Dataset Schema: {full_dataset_ref}\n\n")
        f.write(
            f"Generated on: {client_override.query('SELECT CURRENT_DATETIME()').result().__next__()[0]}\n\n"
        )
        f.write("## Table of Contents\n\n")
